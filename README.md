...
Spark 체크포인트가 컨테이너 내부(휘발성 레이어)에만 있어, `spark` 컨테이너를 재시작할 때마다 Kafka `earliest` 오프셋부터 전체를 다시 읽고 있었습니다. 체크포인트 디렉터리를 호스트 볼륨으로 마운트해 해결했습니다.

> **이후 변경(2026-09-09)**: 데이터 수명 정책을 "세션마다 초기화, MinIO만 영구 보존"으로 정하면서 마운트 방식을 **호스트 바인드 → Docker 익명 볼륨**으로 바꿨습니다. 익명 볼륨은 컨테이너가 *재생성*될 때 비고 *재시작*할 때는 유지되므로, 위 문제(재시작 시 유실)는 그대로 막으면서 세션 경계에서는 깨끗하게 시작합니다. 아래 수치는 최초 해결 시점의 실측이며 현재 구조에서도 동일하게 유지됩니다.
...
1번에서 체크포인트를 영속화했더니, 그 조치의 2차 부작용이 드러났습니다. Kafka는 1시간 retention이라 오래된 메시지를 지우는데, 체크포인트는 그대로 남아 이미 삭제된 오프셋을 가리킵니다. Spark 기본값(`failOnDataLoss=true`)은 이를 치명적 오류로 처리해 스트림을 종료합니다. 즉 **스택을 1시간 넘게 꺼뒀다 켜면 Spark가 아예 뜨지 않는** 상태였습니다.
...
> **근거 변경(2026-09-09)**: 세션 시작 초기화를 도입하면서 체크포인트와 Kafka가 항상 함께 비워지므로, 위의 **세션 간 시나리오("꺼뒀다 켜면")는 이제 발생하지 않습니다.** 그럼에도 이 옵션을 유지하는 이유는 세션 *내* 경우가 남기 때문입니다 — spark가 1시간 넘게 죽어 있는 동안 producer가 계속 쓰면 retention이 앞질러 갑니다. 실제로 같은 날 OOM으로 spark가 장시간 내려간 적이 있어 가상의 경우가 아닙니다.
